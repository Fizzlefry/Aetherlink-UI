...
agent_alerts_total = Counter(
    "aether_agent_alerts_total",
    "Total alerts received by agent",
    ["alertname", "severity"]
)

agent_health_checks_total = Counter(
    "aether_agent_health_checks_total",
...
class ActionRequest(BaseModel):
    ...
    service: Optional[str] = Field(None, description="Service name (for scale_up)")
    replicas: Optional[int] = Field(None, description="Target replicas (for scale_up)")
...
async def health():
    ...
    return HealthResponse(
        ...
        consecutive_low=consecutive_low_count
    )
...
async def perform_action(request: ActionRequest):
    ...
    if request.action == "restart_api":
        ...
        if not request.service or not request.replicas:
            raise HTTPException(
                status_code=400,
                detail="scale_up requires 'service' and 'replicas' parameters"
            )
...
async def alertmanager_webhook(webhook: AlertmanagerWebhook):
    ...
    for alert in webhook.alerts:
        ...
        if alertname == "HealthScoreDegradation":
            logger.warning(
                f"Health Score Degradation Alert: tenant={tenant}, severity={severity}"
            )
