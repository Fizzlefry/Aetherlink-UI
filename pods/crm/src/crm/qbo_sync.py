...
from .models_v2 import Lead, Customer, PortalActivity
from .metrics import (
...
def _get_qbo_config(db: Session, org_id: int) -> Optional[dict]:
...
def ensure_customer_in_qbo(db: Session, org_id: int, customer: Customer) -> Optional[str]:
    """
    ...
    try:
        ...
        if customer.qbo_customer_id:
            ...
            customer.qbo_last_sync_at = datetime.now(timezone.utc)
            db.commit()
...
def poll_invoice_status(db: Session, org_id: int, proposal_id: int) -> Tuple[bool, Optional[str]]:
    """
    ...
    try:
        ...
        proposal.qbo_last_sync_at = datetime.now(timezone.utc)
