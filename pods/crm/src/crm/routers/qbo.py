...
QBO_INVOICES_TOTAL = Counter(
    "crm_invoices_generated_total", "Invoices created in QuickBooks Online", ["org_id"]
)

QBO_ERRORS_TOTAL = Counter(
    "crm_qbo_api_errors_total", "QuickBooks Online API failures", ["org_id", "op"]
)
...
def qbo_oauth_start(user=Depends(get_current_user)):
    ...
    if not c["client_id"] or not c["client_secret"]:
        raise HTTPException(
            400,
            "QBO client_id/client_secret not configured. Set QBO_CLIENT_ID and QBO_CLIENT_SECRET environment variables.",
        )

    sess = OAuth2Session(
        c["client_id"], c["client_secret"], scope=SCOPES, redirect_uri=c["redirect_uri"]
    )
...
def qbo_oauth_callback(
    ...
    try:
        sess = OAuth2Session(c["client_id"], c["client_secret"], redirect_uri=c["redirect_uri"])
        token = sess.fetch_token(QBO_TOKEN_URL, code=code, grant_type="authorization_code")
        ...
        return {"status": "connected", "realm_id": realm_id, "env": c["env"], "org_id": user.org_id}
    except Exception as e:
...
@router.get("/status")
def qbo_connection_status(db: Session = Depends(get_db), user=Depends(get_current_user)):
            ...
            "Line": [
                {
                    "DetailType": "SalesItemLineDetail",
                    "Amount": amount_usd,
                    "SalesItemLineDetail": {"ItemRef": {"value": "1", "name": c["item_name"]}},
                }
            ],
        }
