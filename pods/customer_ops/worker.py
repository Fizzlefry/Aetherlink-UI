...
from pods.customer_ops.pii import parse_pii_config, redact_text
...
PII_PLACEHOLDERS_STR = os.getenv(
    "PII_PLACEHOLDERS", "email:[EMAIL],phone:[PHONE],ssn:[SSN],cc:[CARD]"
)
PII_TYPES, PII_PLACEHOLDERS = parse_pii_config(PII_REDACT_TYPES_STR, PII_PLACEHOLDERS_STR)

log.info(
    "PII Redaction: %s (types: %s)",
    "ENABLED" if PII_REDACT_ENABLED else "DISABLED",
    sorted(PII_TYPES) if PII_REDACT_ENABLED else "N/A",
)
...
def ingest_text_job(text: str, source: str, tenant: str = "default") -> dict[str, Any]:
    ...
    try:
        ...
        for i, (chunk_text, embedding) in enumerate(zip(chunks, vectors, strict=False)):
            ...
            duck_chunks.append(
                {"id": chunk_id, "content": chunk_text, "embedding": embedding, "metadata": {}}
            )
        ...
        log.info(
            "ingest_text_job done: tenant=%s source=%s chunks=%d latency_ms=%.1f",
            tenant,
            source,
            len(chunks),
            dt,
...
def ingest_url_job(url: str, source: str, tenant: str = "default") -> dict[str, Any]:
    ...
    try:
        ...
        if readable and readable.get("text"):
            ...
            log.info(
                "Extracted readable content: title=%s lang=%s",
                metadata.get("title"),
                metadata.get("lang"),
            )
        elif raw_fallback:
        ...
        log.info(
            "ingest_url_job done: url=%s chunks=%s extraction=%s",
            url,
            result.get("ingested_chunks"),
...
def ingest_file_job(
    file_bytes: bytes, filename: str, source: str, tenant: str = "default"
) -> dict[str, Any]:
    ...
    try:
        ...
        log.info(
            "ingest_file_job done: filename=%s chunks=%s", filename, result.get("ingested_chunks")
        )
