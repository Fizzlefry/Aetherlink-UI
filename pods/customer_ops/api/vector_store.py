...
class SQLiteVectorStore:
    ...
    def upsert(self, tenant: str, source: Optional[str], chunks: List[str], vectors: List[List[float]]):
        now = time.time()
        with self.conn:
    ...
    def search(self, tenant: str, query_vec: List[float], top_k: int = 4, min_score: float = 0.15) -> List[Tuple[float, str, Optional[str]]]:
        rows = self.conn.execute("SELECT chunk, vector, source FROM knowledge WHERE tenant = ?", (tenant,)).fetchall()
    ...
    def list(self, tenant: str, limit: int = 50, q: Optional[str] = None):
    ...
    def project_umap(self, tenant: str, k: int = 200):
        ...
        X = np.stack(embs)
        if has_umap:
            reducer = umap.UMAP(n_neighbors=min(10, len(embs)-1), min_dist=0.15, metric="cosine", n_components=2)
        ...
        pts = []
        for (rid, source, text), (x, y) in zip(meta, Y):
            pts.append({
                "id": rid,
                "source": source,
                "text": text[:160],
