...
def test_admin_without_key():
    ...
    try:
        ...
        if r.status_code == 403:
            print("    ✅ Blocked (403): Admin protection working")
            return True
        else:
...
def test_file_upload():
    ...
    try:
        ...
        r = requests.post(
            f"{BASE_URL}/knowledge/ingest-file", files=files, data=data, headers=headers
        )
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if "999" in str(result.get("reply", "")):
                print("    ✅ RAG working: Found Lead 999 in response")
                return True
            else:
                print("    ⚠️  Response received but no Lead 999 mention")
                return True
        else:
