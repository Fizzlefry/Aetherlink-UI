...
def test_async_text_ingest():
    ...
    r = requests.post(
        f"{BASE}/knowledge/ingest-text-async", json=payload, headers=headers, timeout=10
    )
    if r.status_code != 200:
...
def test_async_url_ingest():
    ...
    payload = {"url": "https://example.com", "source": "web_test"}

    r = requests.post(
        f"{BASE}/knowledge/ingest-url-async", json=payload, headers=headers, timeout=10
    )
    if r.status_code != 200:
        print(f"    ❌ Failed to enqueue: {r.status_code} - {r.text}")
        return False
...
def test_async_file_ingest():
    ...
    # Create test file
    files = {
        "file": ("test.txt", b"Async file test content: lead #888 needs follow-up", "text/plain")
    }
    data = {"source": "file_test"}

    r = requests.post(
        f"{BASE}/knowledge/ingest-file-async", files=files, data=data, headers=headers, timeout=10
    )
    if r.status_code != 200:
        print(f"    ❌ Failed to enqueue: {r.status_code} - {r.text}")
        return False
