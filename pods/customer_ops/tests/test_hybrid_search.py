...
def test_semantic_search():
    ...
    response = client.get(
        "/search?q=roof%20installation&k=3&mode=semantic", headers={"x-api-key": TEST_KEY}
    )
...
def test_lexical_search():
    ...
    response = client.get(
        "/search?q=storm%20collar&k=3&mode=lexical", headers={"x-api-key": TEST_KEY}
    )
    ...
    if data["results"]:
        ...
        assert (
            "storm" in content_lower or "collar" in content_lower
...
def test_hybrid_search_default():
    ...
    response = client.get("/search?q=installation%20guide&k=3", headers={"x-api-key": TEST_KEY})
...
def test_hybrid_search_explicit():
    ...
    response = client.get("/search?q=metal%20roof&k=5&mode=hybrid", headers={"x-api-key": TEST_KEY})
...
def test_invalid_mode():
    ...
    response = client.get("/search?q=test&mode=invalid", headers={"x-api-key": TEST_KEY})
...
def test_hybrid_finds_keyword_matches():
    ...
    response = client.get(
        "/search?q=underlayment%20warranty&k=5&mode=hybrid", headers={"x-api-key": TEST_KEY}
    )
...
def test_rate_limiting():
    ...
    for _ in range(65):  # Over the 60/min limit
        response = client.get("/search?q=test&k=1", headers={"x-api-key": TEST_KEY})
