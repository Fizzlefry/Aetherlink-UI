...
from api.main import CONVERSION_RATE, OUTCOME_TOTAL, create_app
...
def test_analytics_outcomes():
    ...
    # Record outcomes
    client.post(
        f"/v1/lead/{leads[0]}/outcome",
        json={"outcome": "booked", "notes": "", "time_to_conversion": 1800},
    )
    client.post(f"/v1/lead/{leads[1]}/outcome", json={"outcome": "ghosted", "notes": ""})
    client.post(
        f"/v1/lead/{leads[2]}/outcome",
        json={"outcome": "booked", "notes": "", "time_to_conversion": 3600},
    )
