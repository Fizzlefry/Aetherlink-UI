...
def test_trafilatura_extracts_clean_text():
    ...
    with patch("requests.get") as mock_get:
        ...
        print(f"    ✅ Boilerplate removed (nav, footer, scripts, sidebar)")
        return True
...
def test_metadata_extraction():
    ...
    with patch("requests.get") as mock_get:
        ...
        if not readable.get("title"):
            print(f"    ⚠️  Title not extracted (may depend on trafilatura version)")
        else:
...
def test_empty_html_fallback():
    ...
    with patch("requests.get") as mock_get:
        ...
        if readable is None and fallback is not None:
            ...
            print(f"    ⚠️  Unexpected result: readable={readable is not None}, fallback={fallback is not None}")
