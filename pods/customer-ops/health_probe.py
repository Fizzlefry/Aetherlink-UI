...
health_checks_total = Counter(
    "health_probe_checks_total", "Total health checks performed", ["endpoint", "status"]
)
health_check_failures = Counter(
    "health_probe_failures_total", "Failed health checks", ["check_type", "reason"]
)
health_check_duration = Histogram(
    "health_probe_duration_seconds", "Health check duration", ["endpoint"]
)
...
current_consumer_count = Gauge(
    "health_probe_consumer_count", "Current consumer count from health check"
)
health_status_gauge = Gauge("health_probe_status", "Overall health status (1=healthy, 0=unhealthy)")


def query_prometheus(query: str, timeout_ms: int | None = None) -> tuple[bool, float]:
    """
    ...
    while retries_left >= 0:
        try:
            ...
            response = requests.get(
                f"{PROMETHEUS_URL}/api/v1/query", params={"query": query}, timeout=timeout_sec
            )
...
def check_health() -> tuple[bool, dict[str, any]]:
    ...
    if success:
        ...
        if not skew_healthy:
            ...
            status["message"] = (
                f"Hot-key skew high: {skew_ratio:.2f}x (threshold: {SKEW_THRESHOLD}x, window: {WINDOW_MINUTES}m)"
            )
            logger.warning(
                json.dumps(
                    {
    ...
    if success:
        ...
        if not consumer_healthy:
            status["healthy"] = False
            status["message"] = (
                f"Under-replicated: {int(consumer_count)} consumers (min: {MIN_CONSUMERS}, window: {WINDOW_MINUTES}m)"
            )
            logger.warning(
                json.dumps(
                    {
            ...
            health_check_failures.labels(
                check_type="consumer_count", reason="under_replicated"
            ).inc()
    else:
        ...
        if success:
            ...
            if not consumer_healthy:
                status["healthy"] = False
                status["message"] = (
                    f"Under-replicated: {int(consumer_count)} consumers (min: {MIN_CONSUMERS})"
                )
        else:
...
def readiness_probe():
    ...
    if is_healthy:
        ...
        logger.info(
            json.dumps({"event": "readiness_check_passed", "window_minutes": WINDOW_MINUTES})
        )
...
def liveness_probe():
    ...
    """
    return jsonify(
        {"status": "alive", "service": "crm-events-sse", "prometheus_url": PROMETHEUS_URL}
    ), 200
