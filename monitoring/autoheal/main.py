...
import requests
from audit import now_ts, write_event
...
CHAOS_FAILURE_RATE = float(
    os.getenv("AUTOHEAL_CHAOS_FAILURE_RATE", "0")
)  # 0 = disabled, 10 = 10% fail
...
ACTION_TOTAL = Counter(
    "autoheal_actions_total", "Autoheal actions executed", ["alertname", "result"]
)
ACTION_LAST_TS = Gauge("autoheal_action_last_timestamp", "Unix ts of last action", ["alertname"])
COOLDOWN_REMAINING = Gauge(
    "autoheal_cooldown_remaining_seconds", "Cooldown remaining", ["alertname"]
)
AUTOHEAL_EVENT_TOTAL = Counter("autoheal_event_total", "Autoheal events by kind", ["kind"])
AUTOHEAL_ACTION_FAILURES_TOTAL = Counter(
    "autoheal_action_failures_total", "Action failures by alertname", ["alertname"]
)
AUTOHEAL_LAST_EVENT_TS = Gauge(
    "autoheal_last_event_timestamp", "Last observed autoheal event (unix ts)"
)
...
def _create_silence(matchers: dict[str, str], duration: str, comment: str):
    ...
    ends_at = datetime.now(UTC) + _parse_duration(duration)
    payload = {
...
async def root():
    ...
    return {
        ...
        "chaos": {"enabled": CHAOS_ENABLED, "failure_rate_pct": CHAOS_FAILURE_RATE},
...
def _iter_audit():
    ...
    try:
        with open(os.getenv("AUTOHEAL_AUDIT_PATH", "/app/audit.jsonl"), encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line:
                    continue
...
def audit(
    ...
    since: float | None = Query(None, description="Unix timestamp filter (events after)"),
    kind: str | None = Query(None, description="Event kind filter"),
    alertname: str | None = Query(None, description="Alert name filter"),
    contains: str | None = Query(None, description="Text search in event JSON"),
):
    """
        ...
        if AUTOHEAL_ENABLED and not AUTOHEAL_DRY_RUN:
            try:
                ...
                if CHAOS_ENABLED and (time.time() % 100) < CHAOS_FAILURE_RATE:
                    ...
                    results.append(
                        {"alert": alertname, "result": "chaos_injected_failure", "rc": rc}
                    )
                else:
                    rc = os.system(cmd)
                    if rc == 0:
