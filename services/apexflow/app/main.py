...
from .models import Appointment, Job, Lead, LeadNote
...
LEADS_ASSIGNED = Counter(
    "apexflow_leads_assigned_total", "Total lead assignments", ["tenant_id", "actor"]
)
...
def get_tenant(
    ...
    if not x_tenant_id and not x_jwt_tenant:
        raise HTTPException(
            status_code=400, detail="tenant required (x-tenant-id header or JWT claim)"
        )

    # If both present, they must match
    if x_tenant_id and x_jwt_tenant and x_tenant_id != x_jwt_tenant:
        raise HTTPException(
            status_code=403, detail=f"tenant mismatch: header='{x_tenant_id}' jwt='{x_jwt_tenant}'"
        )
...
def create_lead(
    ...
    try:
        ...
        if "ix_leads_tenant_email_unique" in str(e):
            raise HTTPException(
                status_code=409, detail=f"Lead with email {payload.email} already exists"
            )
...
def list_owners(tenant: str = Depends(get_tenant), user: dict = Depends(get_user)):
    """
...
def update_lead(
    ...
    try:
        ...
        from .kafka import publish_lead_assigned, publish_lead_status_changed
        ...
        if payload.assigned_to is not None and payload.assigned_to != old_assigned_to:
            publish_lead_assigned(
                lead_id=lead.id, assigned_to=payload.assigned_to, actor=actor, tenant_id=tenant
            )
            ...
            system_note = LeadNote(tenant_id=tenant, lead_id=lead.id, body=note_body, author=actor)
            db.add(system_note)
            db.commit()
...
def list_jobs(
    db: Session = Depends(get_db), tenant: str = Depends(get_tenant), user: dict = Depends(get_user)
):
...
def list_appts(
    ...
    appts = (
        ...
        .filter(Appointment.tenant_id == tenant)
...
def get_appt(
    ...
    appt = (
        db.query(Appointment)
        .filter(Appointment.id == appt_id, Appointment.tenant_id == tenant)
        .first()
    )
    if not appt:
...
def create_lead_note(
    ...
    publish_lead_note_added(
        lead_id=lead_id, note_id=db_note.id, body=note.body, author=actor, tenant_id=tenant
    )
...
def get_lead_activity(
    ...
    notes = (
        db.query(LeadNote).filter(LeadNote.tenant_id == tenant, LeadNote.lead_id == lead_id).all()
    )
