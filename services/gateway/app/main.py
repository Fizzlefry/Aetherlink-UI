...
auth_failures = Counter("gateway_auth_failures_total", "Total authentication failures", ["reason"])
...
def verify_jwt(token: str) -> dict:
    ...
    try:
        ...
        if not (
            ...
            or actual_issuer.replace("localhost:8180", "keycloak:8080") == expected_issuer
        ):
...
async def auth_and_proxy_middleware(request: Request, call_next):
    ...
    if auth_header and auth_header.lower().startswith("bearer "):
        ...
        try:
            ...
            logger.info(
                f"JWT verified - user: {claims.get('preferred_username')}, tenant: {tenant_id}"
            )
    ...
    try:
        ...
        if FORWARD_CLAIMS and claims:
            ...
            upstream_headers[f"{CLAIM_HEADER_PREFIX}username"] = claims.get(
                "preferred_username", ""
            )
...
def who_am_i(authorization: str | None = Header(None)):
    """
