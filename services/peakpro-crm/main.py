...
def ai_snapshot():
    ...
    with get_db() as conn:
        ...
        deals = conn.execute(
            "SELECT * FROM deals ORDER BY updated_at DESC LIMIT 25"
        ).fetchall()
        notes = conn.execute(
            "SELECT * FROM notes ORDER BY created_at DESC LIMIT 25"
        ).fetchall()
    ...
    return {
        ...
        "deals": {
            ...
            "stale": stale_deals[:3],
            "total_value": sum(d.get("value", 0) for d in deals_list if d.get("stage") not in ["lost"])
        },
        "notes": {
...
def create_contact(
    ...
    with get_db() as conn:
        print(f"[PeakPro CRM] Executing INSERT with values: {contact.name}, {contact.email}, {contact.phone}, {contact.company}, {now}, {used_key}")
        cur = conn.execute(
...
def create_deal(deal: Deal, request: Request):
    ...
    with get_db() as conn:
        cur = conn.execute(
            ...
            (deal.title, deal.value, deal.stage, deal.probability, deal.contact_id, now, now, used_key)
        )
