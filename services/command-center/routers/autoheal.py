...
async def list_autoheal_rules(enabled: bool | None = None, limit: int = 100, offset: int = 0):
    ...
    for rule_id, rule in AUTOHEAL_LIMITS.items():
        rule_obj = AutohealRule(
            ...
            enabled=True,
            cooldown_seconds=rule.get("cooldown_minutes", 0) * 60 if rule.get("cooldown_minutes") else None,
...
from autoheal import run_autoheal_cycle, get_healing_history, clear_endpoint_cooldown
from autoheal.engine import AutohealResult
...
async def trigger_autoheal_cycle(
    dry_run: Optional[bool] = Query(None, description="If true, predict actions without executing"),
    ...
    **Status Codes:**
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Auto-healing cycle failed: {str(e)}"
        )
