...
async def get_insights_summary(
    ...
    delivery_stats = {
        ...
        "success_rate": round(status_counts.get("delivered", 0) / total_deliveries * 100, 1)
    ...
    successful_replays = [d for d in replayed_deliveries if d.get("status") == "delivered"]

    replay_stats = {
        "total_replayed": len(replayed_deliveries),
        "replay_successes": len(successful_replays),
        "replay_success_rate": round(len(successful_replays) / len(replayed_deliveries) * 100, 1)
        if replayed_deliveries
        else 0.0,
    }
    ...
    transient_deliveries = [
        d for d in deliveries if d.get("triage_label") == "transient_endpoint_down"
    ]
    transient_successes = [
        d
        for d in transient_deliveries
        if d.get("attempts", 0) > 1 and d.get("status") == "delivered"
    ]
...
async def get_insights_trends(
    ...
    for i in range(num_intervals):
        ...
        interval_deliveries = [
            ...
            for d in deliveries
            if d.get("created_at")
            and interval_start.isoformat() <= d["created_at"] < interval_end.isoformat()
        ]

        # Calculate metrics for this interval
        total = len(interval_deliveries)
        failures = sum(
            1 for d in interval_deliveries if d.get("status") in ("failed", "dead_letter")
        )
