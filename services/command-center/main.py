...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Phase XIII: Load persisted state from disk
        # PERSISTENCE-LAYER V1 (JSON)
        # To move to Postgres, replace this section with DB queries
        try:
            persisted_schedules = load_json_self_heal(SCHEDULES_FILE, {})
            ACCU_IMPORT_SCHEDULES.update(persisted_schedules)
            print(f"[persist] Loaded {len(persisted_schedules)} schedules from disk")
        except Exception as e:
            print(f"[persist] Failed to load schedules: {e}")

        try:
            persisted_audit = load_json_self_heal(AUDIT_FILE, [])
            # Keep only the last 100 like before
            ACCU_SCHEDULER_AUDIT.extend(persisted_audit[-MAX_AUDIT_ENTRIES:])
            print(f"[persist] Loaded {len(persisted_audit)} audit entries from disk")
        except Exception as e:
            print(f"[persist] Failed to load audit: {e}")

        try:
            persisted_runs = load_json_self_heal(LOCAL_RUNS_FILE, [])
            LOCAL_ACTION_RUNS.extend(persisted_runs[-MAX_LOCAL_ACTION_RUNS:])
            print(f"[persist] Loaded {len(persisted_runs)} local action runs from disk")
        except Exception as e:
            print(f"[persist] Failed to load local runs: {e}")
        ...
        if REPLICATION_ENABLED:
            try:
                ...
                print(f"[startup] Replication enabled: {REPLICA_URL}")
            except Exception as e:
...
app = FastAPI(title="AetherLink Command Center", version="0.1.0", lifespan=lifespan)
...
async def retention_worker():
    ...
    while True:
        try:
            ...
            results = []  # TODO: implement event_store
...
def log_scheduler_audit(
    ...
    """
    Record operator action in audit trail with structured format.

    Phase XIV: Enhanced with consistent operation names and source tracking.
    PERSISTENCE-LAYER V1 (JSON)
    To move to Postgres, replace:
    - log_scheduler_audit() write
    - scheduler state saves (ACCU_IMPORT_SCHEDULES)
    - audit load on startup
    """
    now_ts = time.time()
    ...
    # ACCU_SCHEDULER_AUDIT is your in-memory list
    ACCU_SCHEDULER_AUDIT.insert(0, entry)

    # cap at 100
    if len(ACCU_SCHEDULER_AUDIT) > MAX_AUDIT_ENTRIES:
        del ACCU_SCHEDULER_AUDIT[MAX_AUDIT_ENTRIES:]
...
async def acculynx_pull_for_tenant(tenant: str):
    ...
    - run-now endpoint
    Responsible for:
    - calling AccuLynx (real or stub)
    - updating ACCU_IMPORT_SCHEDULES
    - persisting
    - auditing
    """
    now_ts = time.time()
    ...
    if tenant not in ACCU_IMPORT_SCHEDULES:
        raise HTTPException(status_code=404, detail=f"Unknown tenant: {tenant}")

    try:
        # 1) call adapter
        api_result = await acculynx_fetch_jobs(ACCU_LYNX_CONFIG, tenant)

        # 2) update in-memory schedule
        sched = ACCU_IMPORT_SCHEDULES[tenant]
        sched["last_run"] = now_ts
        sched["last_run_iso"] = now_iso
        sched["last_status"] = {
            "ok": bool(api_result.get("ok")),
            "ts": now_ts,
            "ts_iso": now_iso,
            "message": api_result.get("message") or "AccuLynx import completed",
            "result": api_result,
        }
        ...
        # 4) audit (reuse enriched audit from XIV)
        log_scheduler_audit(
            tenant=tenant,
            operation="scheduler.import.completed",
            source="scheduler",
            metadata={
                "ok": bool(api_result.get("ok")),
        ...
        ACCU_IMPORT_SCHEDULES[tenant]["last_run"] = now_ts
        ACCU_IMPORT_SCHEDULES[tenant]["last_status"] = {
            "ok": False,
            "ts": now_ts,
            "ts_iso": now_ts,
            "message": f"AccuLynx import failed: {e}",
        }
        ...
        log_scheduler_audit(
            tenant, "scheduler.import.failed", source="scheduler", metadata={"error": str(e)}
        )
        await emit_event(
...
async def ops_ping():
    ...
    try:
        ...
        print(f"Exception in ops_ping: {e}")
        import traceback
...
async def acculynx_pause(request: Request):
    ...
    try:
        ...
        log_scheduler_audit(tenant, "scheduler.schedule.paused", source="ui")
...
async def acculynx_resume(request: Request):
    ...
    try:
        ...
        log_scheduler_audit(tenant, "scheduler.schedule.resumed", source="ui")
...
async def acculynx_run_now(request: Request):
    ...
    Phase XI: Now logs to audit trail.
    Phase XII: Protected with RBAC - requires x-ops header.
    Phase XIV: Enhanced audit with force_run operation and ui source.
    Header: x-tenant (default: the-expert-co)
    """
    ensure_ops(request)  # Phase XII: Require operator privileges
    try:
        tenant = request.headers.get("x-tenant", "the-expert-co").strip().lower()
        now_ts = time.time()
        now_iso = to_iso(now_ts)

        # Phase XIV: Enhanced audit logging with ui source
        log_scheduler_audit(
            tenant, "scheduler.import.force_run", source="ui", metadata={"ts_iso": now_iso}
        )
        ...
        # Phase XIV: Return enhanced response with timestamps
        return {
            "ok": True,
            "tenant": tenant,
            "ts": now_ts,
            "ts_iso": now_iso,
            "message": f"Force run completed for {tenant}",
            "result": result,
        }
    except Exception as e:
...
async def acculynx_delete_schedule(request: Request):
    ...
    try:
        ...
        if tenant in ACCU_IMPORT_SCHEDULES:
            ...
            log_scheduler_audit(tenant, "scheduler.schedule.deleted", source="ui")
